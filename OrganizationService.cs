...
namespace Services
{
    public class OrganizationService : IOrganizationService
    {
        ...
        public Organization SelectById(int id)
        {
            ...
            _data.ExecuteCmd("[dbo].[Organizations_Select_ById]", delegate (SqlParameterCollection parameterCollection)
            {
        ...
        public Paged<Organization> SelectAll_Paginated(int pageIndex, int pageSize)
        {
            ...
            int totalCount = 0;
            _data.ExecuteCmd("[dbo].[Organizations_SelectAll_Paginated]", delegate (SqlParameterCollection parameterCollection)
            {
        ...
        public Paged<Organization> SelectByCreatedBy_Paginated(int pageIndex, int pageSize, int currentUser)
        {
            ...
            _data.ExecuteCmd("[dbo].[Organizations_Select_ByCreatedBy_Paginated]", delegate (SqlParameterCollection parameterCollection)
            {
                parameterCollection.AddWithValue("@PageIndex", pageIndex);
                parameterCollection.AddWithValue("@PageSize", pageSize);
                parameterCollection.AddWithValue("@CreatedBy", currentUser);
            },
        ...
        {
            _data.ExecuteNonQuery("[dbo].[Organizations_Delete_ById]", delegate (SqlParameterCollection parameterCollection)
            {
                parameterCollection.AddWithValue("@Id", id);
            },
            returnParameters: null);
